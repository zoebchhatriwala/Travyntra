    ...
    return (
        <div className={cn(
            ...
            isFullscreen
                ...
                : "h-[750px] rounded-corner-xl"
        )}>
            {/* Header */}
            <div className="p-6 border-b border-gray-200 bg-white/80 backdrop-blur-sm flex items-center justify-between">
                <div>
                ...
                <Button
                    ...
                    className="text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-corner-md"
                >
            ...
            <div className="flex-1 overflow-y-auto bg-white p-6 space-y-4" ref={scrollRef}>
                {messages.length === 0 ? (
                    ...
                    messages.map((msg) => {
                        ...
                        return (
                            <div key={msg.id} className={cn("flex gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300", isMe ? "flex-row-reverse" : "flex-row")}>
                                <div className={cn(
                                ...
                                <div className={cn("flex flex-col gap-1.5 max-w-[75%]", isMe && "items-end")}>
                                    ...
                                    <div className={cn(
                                        ...
                                        isMe
                                            ? "bg-gradient-to-br from-indigo-600 to-indigo-700 text-white rounded-corner-lg rounded-tr-md"
                                            : isAgent
                                                ? "bg-amber-50/80 text-gray-900 rounded-corner-lg rounded-tl-md border border-amber-200"
                                                : "bg-white text-gray-800 rounded-corner-lg rounded-tl-md border border-gray-100",
                                            ...
                                            <div className={cn("space-y-2", text && "mt-3")}>
                                                {msgAttachments.map((att, idx) => (
                                                    <a
                                                        ...
                                                        className={cn(
                                                            "flex items-center gap-4 px-3 py-2.5 rounded-corner-md text-xs font-medium transition-all group",
                                                            isMe
                                                    ...
                                                    >
                                                        <div className={cn(
                                                            "w-8 h-8 rounded-corner-sm flex items-center justify-center",
            ...
            <div className="p-4 bg-white border-t border-gray-200">
                ...
                {attachments.length > 0 && (
                    <div className="mb-3 flex flex-wrap gap-2 animate-in slide-in-from-bottom-2 duration-200">
                        {attachments.map((file, index) => (
                            <div key={index} className="flex items-center gap-2 bg-indigo-50 border border-indigo-200 rounded-corner-md px-3 py-2 text-xs group hover:bg-indigo-100 transition-colors">
                                <div className="w-6 h-6 rounded-corner-sm bg-indigo-100 flex items-center justify-center">
                ...
                {showMentions && filteredUsers.length > 0 && (
                    <div className="mb-3 bg-white border border-gray-200 rounded-corner-lg shadow-xl max-h-48 overflow-y-auto animate-in slide-in-from-bottom-2 duration-200">
                        {filteredUsers.map(user => (
                            <button
                                key={user.id}
                                onClick={() => handleMention(user.name || 'User')}
                                className="w-full px-4 py-3 text-left text-sm hover:bg-indigo-50 flex items-center gap-4 transition-colors first:rounded-t-2xl last:rounded-b-2xl"
                            >
                ...
                <div className="flex gap-2 items-end">
                    ...
                    <Button
                        ...
                        className="flex-shrink-0 h-11 w-11 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-corner-md transition-all"
                    >
                        <Paperclip size={20} />
                    </Button>
                    <div className="flex-1 relative">
                        <Textarea
                            ...
                            className="min-h-[52px] max-h-[120px] pr-16 pl-4 py-4 bg-gray-50 resize-none border-gray-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 rounded-corner-lg text-sm placeholder:text-gray-400 leading-relaxed"
                            onKeyDown={(e) => {
                        ...
                        <Button
                            ...
                            className="absolute right-2 bottom-2 h-10 w-10 bg-gradient-to-br from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white rounded-corner-md shadow-lg transition-all hover:scale-105 disabled:opacity-50 disabled:hover:scale-100"
                        >
