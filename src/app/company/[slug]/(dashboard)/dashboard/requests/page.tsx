        ...
        <div className="p-8 space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <Button size="icon" className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-corner-md shadow-lg shadow-indigo-200" asChild>
            ...
            {requests.length === 0 ? (
                <div className="text-center py-20 bg-gray-50/50 rounded-corner-xl border-2 border-dashed border-gray-100">
                    ...
                    {!query && (
                        <Button className="bg-indigo-600 text-white rounded-corner-md font-bold text-xs shadow-lg shadow-indigo-200" asChild>
                        ...
                        <Link key={req.id} href={`/company/${slug}/dashboard/requests/${req.id}`}>
                            <div className="flex items-center justify-between p-6 bg-white rounded-corner-xl border border-gray-100 hover:shadow-md hover:border-indigo-100 transition-all group">
                                <div className="flex items-center gap-6 min-w-0 flex-1 mr-4">
                                    <div className="w-12 h-12 bg-gray-50 rounded-corner-lg flex items-center justify-center text-gray-400 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors shrink-0">
                                        <Plane size={24} />
                                    </div>
                                ...
                                <div className="flex items-center gap-6 shrink-0">
                                    {req.isCollaborator && (
                                        <Badge variant="outline" className="rounded-corner-md px-3 py-1 font-bold text-[10px] border-indigo-100 bg-indigo-50/50 text-indigo-600">
                                            Shared with me
                                        </Badge>
                                    )}
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-center gap-2">
                    <Button
                        variant="outline"
                        size="icon"
                        className={cn("rounded-corner-md w-10 h-10", currentPage <= 1 && "pointer-events-none opacity-50")}
                        asChild
                    >
                    ...
                    <Button
                        variant="outline"
                        size="icon"
                        className={cn("rounded-corner-md w-10 h-10", currentPage >= totalPages && "pointer-events-none opacity-50")}
                        asChild
                    >
