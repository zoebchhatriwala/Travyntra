    ...
    return (
        <div className="p-8 space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <Button size="lg" className="bg-primary hover:bg-indigo-700 text-white rounded-corner-lg font-bold shadow-lg shadow-indigo-200 transition-all hover:scale-105" asChild>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Active Requests */}
                <Card className="border-none shadow-joy rounded-corner-xl bg-blue-50/50">
                    <CardContent className="p-6 flex items-center gap-4">
                        <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-corner-lg flex items-center justify-center">
                            <Plane size={24} />
                        </div>
                        <div>
                ...
                {/* Completed Trips */}
                <Card className="border-none shadow-joy rounded-corner-xl bg-emerald-50/50">
                    <CardContent className="p-6 flex items-center gap-4">
                        <div className="w-12 h-12 bg-emerald-100 text-emerald-600 rounded-corner-lg flex items-center justify-center">
                ...
                <Link href={`/company/${slug}/dashboard/assets`}>
                    <Card className="border-none shadow-joy rounded-corner-xl bg-white hover:ring-indigo-200 transition-all cursor-pointer h-full group">
                        <CardContent className="p-6 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-indigo-50 text-primary rounded-corner-lg flex items-center justify-center group-hover:scale-110 transition-transform">
                                    <FileText size={24} />
                                </div>
                                <div>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    ...
                    <Button variant="ghost" asChild className="text-xs font-black text-primary uppercase tracking-widest hover:bg-indigo-50">
                ...
                {stats?.recentRequests && stats.recentRequests.length > 0 ? (
                    <div className="grid gap-4">
                        {stats.recentRequests.map((req) => (
                            <Link key={req.id} href={`/company/${slug}/dashboard/requests/${req.id}`}>
                                <div className="flex items-center justify-between p-6 bg-white rounded-corner-xl border border-gray-100 hover:shadow-md hover:border-indigo-100 transition-all group">
                                    <div className="flex items-center gap-6 min-w-0 flex-1 mr-4">
                                        <div className="w-12 h-12 bg-gray-50 rounded-corner-lg flex items-center justify-center text-gray-400 group-hover:bg-indigo-50 group-hover:text-primary transition-colors shrink-0">
                                            <Plane size={24} />
                                        </div>
                                        <div className="min-w-0 flex-1">
                                            <Tooltip>
                                                <TooltipTrigger asChild>
                                                    <h4 className="text-base font-bold text-gray-900 mb-1 group-hover:text-primary transition-colors break-all">{req.title}</h4>
                                                </TooltipTrigger>
                                                <TooltipContent>
                                                    <p>{req.title}</p>
                                                </TooltipContent>
                                    ...
                                    <div className="flex items-center gap-6 shrink-0">
                                        {req.isCollaborator && (
                                            <Badge variant="outline" className="rounded-corner-md px-3 py-1 font-bold text-[10px] border-indigo-100 bg-indigo-50/50 text-primary">
                                                Shared with me
                                            </Badge>
                                        )}
                                        {req.budget > 0 && (
                    ...
                    <div className="text-center py-20 bg-gray-50/50 rounded-corner-xl border-2 border-dashed border-gray-100">
                        ...
                        <Button className="bg-primary text-white rounded-corner-md font-bold text-xs shadow-lg shadow-indigo-200" asChild>
