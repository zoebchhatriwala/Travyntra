...
import { AssetTypeFilter } from "./_components/asset-type-filter";

type SearchParams = Promise<{ [key: string]: string | string[] | undefined }>;

export default async function AssetsPage(props: { searchParams: SearchParams }) {
    ...
    const { documents: assets, total } = await getEmployeeAssets({
        page,
        limit,
        query,
        type
            ...
            {assets.length === 0 ? (
                <div className="text-center py-20 bg-gray-50/50 rounded-[32px] border-2 border-dashed border-gray-100">
                    ...
                    <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest mb-1">
                        {query || type !== "ALL" ? "No matching documents" : "Vault is empty"}
                    </h3>
                ...
                <>
                    ...
                    <div className="mt-8">
                        <PaginationControls totalCount={total} pageSize={limit} />
                    </div>
                </>
            )}
        </div>
